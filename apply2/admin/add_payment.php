...
$uid = $_GET['uid'];
$cc = mysql_query("select * from `occ_registrant` where `source_type` = 2");
...
if($isPosted > 0)
	{
		...
		$insertQuery = mysql_query("insert into payment_recieved values ('','$user_id','$amount','$description','$note','$status','$method','$datetwin','','2')");
		...
		$findpay = mysql_query("SELECT * FROM  payment_recieved WHERE payment_id =  '$payment_id'");
				

		$finduser = mysql_query("SELECT * FROM  occ_registrant where sid = '$user_id'");
...
<p>You have a pending payment of &pound;".$item_amount." for <strong>\"".$item_desc."\"</strong> on your account. Please click the PAY NOW button below to pay the outstanding amount.</p>

<p>
<a href=\"https://www.paypal.com/cgi-bin/webscr?business=omer@osterleycc.com&cmd=_xclick&currency_code=GBP&amount=".$item_amount."&item_name=".$item_desc."&custom=".$item_custom."\" ><img src=\"http://www.osterleycc.com/apply/images/paynow.gif\"  />
</a>
...
<p>Osterley Cricket Club</p>
	...
	//					$mail_function = mail("qasimnepster@gmail.com", " Registration Successful ", $body, $headers); 
		...
		if($insertQuery)
		{
			mysql_query("update `occ_registrant` set `status` = 1 where `sid` = '$uid' WHERE `source_type` = 2");
			header("Location:adutls_info.php?action=Payment Added Successfully");
		}
		else
		{
			echo mysql_error();
...
<td style="background:#fff;  border-bottom:none;">
<select name="status" id="status" class="fields" style="width:52%;">
