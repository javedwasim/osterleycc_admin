...
if($userId)
{
	...
	$selectusers = mysql_query("select * from occ_registrant as r, payment_recieved as p where r.sid = p.uid and p.uid = '$userId' and p.payment_id = '$payId' and p.source_type = 2");
	...
	if($isPosted > 0)
		{
			...
			if($_POST['email_format']!= NULL)
			{
				...
				if($formats == '1')
					...
					//	$NameofPerson = "TEST NAME";					
						$body = readTemplateFile("AdultsWelcomeEmail.html");
					...
					//	$mail_function = mail("omer@osterleycc.com", " Registration Successful ", $body, $headers);
						$mail_function = mail($user_conf_email, " Registration Successful ", $body, $headers); 
					}
				if($formats == '2')
					{
						$NameofPerson = $rows['occ_firstname'].'&nbsp;'.$rows['occ_lastname'];
			...
			header("Location:adutls_info.php?action= Updates Successfully");
